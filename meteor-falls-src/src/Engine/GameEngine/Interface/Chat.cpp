...
Chat::Chat(GameEngine* game):m_game_engine(game)
{
    ...
    m_show_message->setSize(CEGUI::UVector2(CEGUI::UDim(0.25, 0), CEGUI::UDim(0.30, 0)));
    m_show_message->hide();

    CEGUI::System::getSingleton().getGUISheet()->addChildWindow(m_show_message);

    m_message = (CEGUI::Editbox*)m_show_message_manager.createWindow("OgreTray/Editbox", "Champ");
    m_message->setSize(CEGUI::UVector2(CEGUI::UDim(0.25, 0), CEGUI::UDim(0.05, 0)));
    m_message->setPosition(CEGUI::UVector2(CEGUI::UDim(0, 0),
                                         CEGUI::UDim((1-m_message->getSize().d_y.d_scale), 0)));
    m_message->activate();
    m_show_message->setPosition(CEGUI::UVector2(CEGUI::UDim(0, 0),
                                         CEGUI::UDim((1-m_show_message->getSize().d_y.d_scale-m_message->getSize().d_y.d_scale), 0)));
    m_message->hide();
    CEGUI::System::getSingleton().getGUISheet()->addChildWindow(m_message);
}

Chat::~Chat()
{
	CEGUI::System::getSingleton().getGUISheet()->removeChildWindow(m_show_message);
	CEGUI::WindowManager::getSingleton().destroyWindow(m_show_message);
	CEGUI::System::getSingleton().getGUISheet()->removeChildWindow(m_message);
