        ...
        case mf::EngineMessageType::SPAWN:
        {
            ...
            else
            {
                    ...
                    j->avatar(message.ints[mf::EngineMessageKey::AVATAR_ID]))
                {
                    ...
                    hero->teleport(message.positions[mf::EngineMessageKey::OBJECT_POSITION]);
                }
                int id = hero->id();
...
void ServerGameEngine::handleBuildRequest(EngineMessage &message)
{
    Equipe *team = getEquipe(message.ints[mf::EngineMessageKey::TEAM_ID]);
    if (team)
    {
        int count = message.ints[mf::EngineMessageKey::COUNT];
        TypeUnite *type = team->faction()->getType(message.ints[mf::EngineMessageKey::OBJECT_TYPE]);
        Unite *builder = team->getUnite(message.ints[mf::EngineMessageKey::BUILDER]);
        Batiment *batimentBuilder = dynamic_cast<Batiment *>(builder);
        if (batimentBuilder)
            build(batimentBuilder, type, count);
    }
}

void ServerGameEngine::build(Batiment *builder, TypeUnite *type, int count)
{
    for (int i = 0; i < count; ++i)
    {
        if (builder->equipe()->canBuild(type))
        {
            Unite *newUnit = builder->equipe()->factory()->create(type);
            newUnit->teleport(builder->position() + Vector3D(0,50,0));
            builder->addBuildTask(newUnit);
            EngineMessage message(nullptr);
            message.message = mf::EngineMessageType::BUILD;
            message.ints[mf::EngineMessageKey::BUILDER] = builder->id();
            message.ints[mf::EngineMessageKey::TEAM_ID] = builder->equipe()->id();
            message.ints[mf::EngineMessageKey::OBJECT_ID] = newUnit->id();
            message.ints[mf::EngineMessageKey::OBJECT_TYPE] = newUnit->type()->id();
            message.positions[mf::EngineMessageKey::OBJECT_POSITION] = newUnit->position();
            ((ServerNetworkEngine *)m_manager->getNetwork())->sendToAllTcp(&message);
        }
        else return;
    }
}
